#pragma once
// MatFree - Runtime value type for the MatFree engine
...
public:
    ...
    // Element access (0-indexed internally, 1-indexed externally)
...
public:
    ...
    // Struct
    explicit Value(MFStruct s) : type_(ValueType::STRUCT), struct_(std::move(s)) {}
    ...
    double scalarDouble() const {
        if (isString()) {
            // Convert single char to its ASCII value
    ...
    const MFStruct& structVal() const {
        if (!isStruct()) throw RuntimeError("Value is not a struct");
        return struct_;
    }
    ...
    static ValuePtr makeStruct(MFStruct s) { return std::make_shared<Value>(std::move(s)); }
